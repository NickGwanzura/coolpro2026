...
import { invites, users, contractorApplications } from '@/db/schema/index';
...
/**
 * Contractor access is invitation-only, mirroring the supplier invite pattern: the admin
 * only identifies the intended recipient (email + region). The invitee sets their own
 * password via /accept-invite, then completes their own profile and questionnaire at
...
export async function POST(req: Request) {
  ...
  const [existingApplication] = await db.select({ id: contractorApplications.id }).from(contractorApplications).where(eq(contractorApplications.email, email)).limit(1);
  if (existingApplication) return NextResponse.json({ error: 'A contractor invite already exists for that email' }, { status: 409 });
  ...
  const emailResult = await sendInviteEmail({ email, inviteUrl, role: 'contractor', invitedBy: session.name });
  await logEmail({
    emailType: 'account_activation', recipientEmail: email, relatedEntityType: 'contractor_invite',
