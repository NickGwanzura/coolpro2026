...
import type { PublicPermitVerification } from '@/types/index';
...
function VerifyPermitContent() {
  ...
  const runSearch = async (permitNumber: string, token?: string | null) => {
    ...
    try {
      ...
      if (!res.ok) {
        setResult(null);
        setError(data.error ?? 'Permit not found.');
      } else {
        setResult(data);
      }
    } catch {
      ...
      <section className="pt-28 sm:pt-32 pb-14">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold tracking-tight" style={{ color: '#1C1917' }}>
            Verify an Import/Export Permit
          </h1>
          <p className="mt-4 text-gray-600 leading-relaxed">
            Confirm a HEVACRAZ/NOU-issued refrigerant trade permit is genuine and currently approved.
          </p>
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Permit number — e.g. PMT-XXXXXXXX"
        ...
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          {searched && !loading && error && (
            <div className="border border-rose-200 bg-rose-50 p-6 text-center">
              ...
              <p className="mt-1 text-sm text-rose-600">
                Only approved permits can be verified here. Check the permit number and try again.
              </p>
            </div>
          )}
          ...
          {result && (
            <div className="border border-emerald-200 bg-white shadow-sm">
              <div className="flex items-center gap-3 border-b border-emerald-100 bg-emerald-50 px-6 py-4">
                ...
                <div>
                  <p className="font-bold text-emerald-900">Valid, Approved Permit</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-6 text-sm">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-gray-400">Permit Number</p>
                  <p className="mt-1 font-mono font-semibold text-gray-900">{result.permitNumber}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="mt-1 font-medium text-gray-900">{formatDate(result.expiryDate)}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {!searched && (
            <div className="border border-dashed border-gray-200 p-10 text-center">
              ...
              <p className="text-sm text-gray-400">Enter a permit number above to verify it.</p>
            </div>
          )}
        </div>
      </section>
